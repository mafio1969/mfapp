...
namespace Image2Helper\View\Helper;
...
class Image2Helper extends Helper
{
    ...
    public function crop($width, $height, $resize = true)
    {
        ...
        if ($resize) {
            $ratio_x = $width / (($this->sizes[0]) ? $this->sizes[0] : 1);
            $ratio_y = $height / (($this->sizes[1]) ? $this->sizes[1] : 1);
            if (($ratio_y) > ($ratio_x)) {
    ...
    {
        $cache_dir = implode(DS, Configure::read('Image2Helper.cacheDir'));
        $cache_dir = WWW_ROOT . $cache_dir . DS;

        $cache_path = $cache_dir . implode('_',
                [$start_x, $start_y, $width, $height, $method, basename($this->serverPath)]);
            ...
            if (!$this->_cacheServerPath) {
                ...
                if (function_exists("imagecreatetruecolor") && ($temp = imagecreatetruecolor($width, $height))) {
                    if (function_exists('imagecolorallocatealpha')) {
                        ...
                        imagecopyresampled($temp, $image, 0, 0, $start_x, $start_y, $width, $height, $this->sizes[0],
                            $this->sizes[1]);
                    } else {
                    ...
                    imagecopyresized($temp, $image, 0, 0, $start_x, $start_y, $width, $height, $this->sizes[0],
                        $this->sizes[1]);
                }
    ...
    public function watermark($watermark_path, $position = 'center', $watermark_absolute_path = false)
    {
        ...
        $cache_path = $cache_dir . Text::slug(basename($watermark_path)) . '_' . $position . '_'
        ...
        imagecopy($original, $watermark_source, $watermark_x, $watermark_y, 0, 0, $watermark->sizes[0],
            $watermark->sizes[1]);
    ...
    public function imagePath()
    {
        ...
        $cache_dir = implode('/', Configure::read('Image2Helper.cacheDir'));
