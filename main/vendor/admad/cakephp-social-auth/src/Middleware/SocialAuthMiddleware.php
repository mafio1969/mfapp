...
class SocialAuthMiddleware implements EventDispatcherInterface
{
    ...
    const SOCIAL_PROFILE_NOT_EXISTS = 'social_profile_not_exists';
    ...
    protected function _handleCallbackAction(ServerRequest $request, Response $response)
    {
        ...
        $lang = $request->getCookie('dataLg');
        if (isset(AppController::$languageMapper[$lang])) {
            I18n::setLocale(AppController::$languageMapper[$lang]);
        }
        if ($profile === self::USERNAME_ALREADY_IN_USE) {
            $messages[] = [
                'message' => __('Email of this account is already in use.'),
                'key' => 'success',
                'element' => 'Flash/success',
                'params' => []
            ];
            $request->getSession()->write('Flash.success', $messages);
            return $response->withLocation(
                Router::url([
                    'plugin' => 'Croogo/Users',
                    'controller' => 'Users',
                    'action' => 'login',
                    'lang' => isset(AppController::$languageMapper[$lang]) ? $lang : ''
                ], true)
            );
        }

        if ($profile === self::SOCIAL_PROFILE_NOT_EXISTS) {
            $messages[] = [
                'message' => __('You must first register with social profile.'),
                'key' => 'success',
                'element' => 'Flash/success',
                'params' => []
            ];
    ...
    protected function _getProfile($providerName, ServerRequest $request)
    {
        ...
        $socialProfileExists = $this->_profileModel->find()
            ->where([
        ...
        $socialAction = $request->getSession()->read(UsersController::SOCIAL_ACTION);

        if ((!isset($socialProfileExists->id) || empty($socialProfileExists->id))
            && $socialAction == UsersController::SOCIAL_ACTION_LOGIN) {
            $request->getSession()->delete(UsersController::SOCIAL_ACTION);
            return self::SOCIAL_PROFILE_NOT_EXISTS;
        }
        ...
        if (isset($userExists->id) && !empty($userExists->id)
            && (!isset($socialProfileExists->id) || empty($socialProfileExists->id))) {
            return self::USERNAME_ALREADY_IN_USE;
        }
        ...
        $profile = $this->_profileModel->find()
            ...
            ->first();
        if ((!isset($profile->id) || empty($profile->id)) && $socialAction == UsersController::SOCIAL_ACTION_LOGIN) {
            $request->getSession()->delete(UsersController::SOCIAL_ACTION);
            return self::SOCIAL_PROFILE_NOT_EXISTS;
        }
